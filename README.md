...
From an active program, **Adatta questa settimana** lets you describe a situation (illness, injury, extra hike, no time, travel, personal conflict). `LLMProvider.adaptWeek` proposes changes to the remaining sessions only. You still have to approve. Hike/Walk and similar Strava sports import as unplanned `other` load (they count toward TSS, they do not match planned workouts).
...
- **Monthly estimate (DeepSeek, amateur athlete):** ~**$0.034 / user / month** for 2 program generations, 16 feedback analyses, 2 performance reports, and 4 week adapts. Ten beta testers ≈ **$0.34 / month**. Figures come from list prices in `lib/llm/constants.ts` and the conservative token scenario in `lib/llm/usage-scenario.ts` (a ceiling, not an invoice). Live totals stay in `LLMInteractionLog`.
